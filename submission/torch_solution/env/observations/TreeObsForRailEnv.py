...
from ..compat import RailAgentStatus

from ...grid_utils import get_new_position
...
class TreeObsForRailEnv(ObservationBuilder):
    ...
    def reset(self):
        self.location_with_agent = -np.ones((self.env.height, self.env.width), dtype=np.int64)
        self._graph_ready = False
        if self.env.rail is not None:
    ...
    def get_many(self, handles: Optional[List[int]] = None) -> Dict[int, Node]:
        ...
        if not self._graph_ready and self.env.rail is not None:
            self.graph.reset(self.env)
            self._graph_ready = True
        ...
        for handle, _agent in enumerate(self.env.agents):
            if _agent.state == RailAgentStatus.ACTIVE:
    ...
    def get(self, handle: int = 0) -> Node:
        ...
        if agent.state in (RailAgentStatus.READY_TO_DEPART, RailAgentStatus.WAITING):
            ...
            agent_virtual_direction = agent.direction if agent.direction is not None else agent.initial_direction
        else:
            return None

        if agent_virtual_position is None or agent_virtual_direction is None:
            return None
    ...
    def _explore_line(self, position, direction):
        ...
        while True:
            if self.location_with_agent[position] != -1:
                ...
                if self.env.agents[other_handle].malfunction_handler.malfunction_down_counter > 0:
...
class Graph():
    ...
    def reset(self, env):
        self.env = env
        self.one_transition = -np.ones((self.env.height, self.env.width, 4), dtype=np.int64)
