...
from .ActorCritic import ActorNet, CriticNet
    ...
    def fast_select_actions(self, handles, state_dict, neighbours_handles, train=True):
        ...
        with torch.no_grad():
            all_states = torch.stack([
                state_dict[handle] if handle != -1 else torch.zeros(self.state_sz, device=self.device)
                for handle in all_handles
            ])
            all_thoughts = self.actor_net.think(all_states)
            for i, h in enumerate(all_handles):
                if h == -1:
                    all_thoughts[i] = torch.zeros(128, dtype=torch.float, device=self.device)
            ...
            if train:
                ...
                actions = torch.argmax(action_distribution.logits, dim=1)
