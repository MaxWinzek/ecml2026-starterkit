...
class GreedyChecker():
    ...
    def on_switch(self, handle):
        ...
        return agent.state == RailAgentStatus.ACTIVE and (*agent.position, agent.direction) in self.switches

    def greedy_position(self, handle):
        agent = self.env.agents[handle]
        if agent.position is None:
            return False
        return (not agent.state in (RailAgentStatus.DONE, RailAgentStatus.DONE_REMOVED, RailAgentStatus.READY_TO_DEPART, RailAgentStatus.WAITING)) \
                and \
                    (not self.on_decision_cell(*agent.position, agent.direction) or agent.malfunction_handler.malfunction_down_counter != 0) \
                and \
    ...
    def reinit_greedy(self):
        ...
        self.decision_cells = np.zeros((self.env.height, self.env.width, 4), dtype=np.bool_)
        ...
        self.location_has_target_array = np.zeros((self.env.height, self.env.width), dtype=np.bool_)
